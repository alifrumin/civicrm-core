...
- **[dev/core#463](https://lab.civicrm.org/dev/core/issues/463) Add in support
  ...
  somewhat - but will not wrap any translatables with ts rather than the
...
- **[dev/core#472](https://lab.civicrm.org/dev/core/issues/472) Allow for
  ...
  This change makes it so that APIv3 Explorer displays wxamples found in an
  extension.
...
- **Translate description in DAO files
  ...
  that if a user uses the getfields api they get the translated description.
...
- **CRM_Core_Resources - Allow container to swap the implementation
  ...
  extension thru the hook hook_civicrm_container.
...
- **Fix typo in hook that enables links to be added to a block
  ...
  create.new.shortcuts in addition to create.new.shorcuts.
...
- **Register "short" and "long" cache services
  ...
  Civi::cache('long') for a long-lived/durability-optimized PSR-16 cache.

- **Remove crm.admin.js
  ([12884](https://github.com/civicrm/civicrm-core/pull/12884))**

  This change is both a performance improvement (removes an unencessary js file)
  and a ui improvement because it improved the icon picker ui.
...
- **Mark event start date as required and refactoring
  ...
  This change ensures that the event start date field has a red * next to it to
...
- **[dev/core#461](https://lab.civicrm.org/dev/core/issues/461) Duplicate
  ...
  would resulting duplicate message templates being saved so that the message
...
- **[CRM-19751](https://issues.civicrm.org/jira/browse/CRM-19751) Once "multiple
  ...
  searching for contacts using the criteria "Opt Out" or "On Hold" was breaking
  so that one can search based on these criteria.
...
- **[dev/core#448](https://lab.civicrm.org/dev/core/issues/448) When creating a
  ...
  Group users who have previously unsubscribed are not included.
...
- **[dev/core#443](https://lab.civicrm.org/dev/core/issues/443) Fix Non-static
  ...
  This change fixes a strict warning thrown for users with a PHP 7.1 set up so
  that no strict warnings are thrown.
...
- **[dev/core#166](https://lab.civicrm.org/dev/core/issues/166)
  ...
  (the user who reported this problem had 1.3 million) the scheduled task to
...
- **[CRM-21311](https://issues.civicrm.org/jira/browse/CRM-21311) Fix
  ...
  card names without breaking contribution forms.
...
- **[dev/financial#23](https://lab.civicrm.org/dev/financial/issues/23)
  ...
  This change removes an enotice when submitting a contribution page with a 0
  transaction amount.
...
- **Fix caching issues with Event permissions
  ...
  are returned as having no permission regardless of what their permissions are.
...
- **Fix payment hangs when registering a user for a paid event without recording
  ...
  This fixes a bug where registering a user for a paid event without recording a
  payment on the back end would result in the payment hanging.
...
- **Fix Activity api to cope with renamed record type labels (e.g renaming
  ...
  This change makes it so the Activity API to filter based on the option name
...
- **Add ts to untranslated string
  ...
  Ensures the text "Error" is translated on error messages thrown when
  processing images on a contact.
...
- **[dev/core#459](https://lab.civicrm.org/dev/core/issues/459)Fix "crmURL"
  ...
  Fixes a bug where some url's were being generated with double & (like &&).
...
- **[dev/core#412](https://lab.civicrm.org/dev/core/issues/412) Avoid truncated
  ...
  to export properly so that one can use the export form with custom group names
  with a UTF-8 character at the 10th character.
...
- **[dev/core#452](https://lab.civicrm.org/dev/core/issues/452) Error when
  ...
  This change fixes a bug for users using PHP 7.1 exporting csv batches would
  crash so that users using PHP 7.1 can export csv batches.
...
- **Status page: hush menu should be on top of other headers
  ...
  menu appears over the status header.
...
- **Correct inconsistent fileID reference
  ...
  contact thru the api would result in a new file entity being created instead
...
- **Contact layout editor support - Skip html encoding rich-text fields
  ...
  being escaped so that the html is preserved as-is.
...
- **[dev/core#418](https://lab.civicrm.org/dev/core/issues/418) ensure all valid
  ...
  This change ensures that when adding a relationship to a group of search
  results Relationships that are any contact type to any contact type show up.
...
- **Do not start session when running under WP-CLI
  ...
  permissions that the shell user does not one would receive a PHP Warning.
...
- **Added a path to the drupal8 path candidates to find civicrm.config.php
  ...
  This change fixes a bug when using the following civibuild command `Civibuild
  create drup8 --type=drupal8-clean` where users would receive a fatal error
...
- **[CRM-17667](https://issues.civicrm.org/jira/browse/CRM-17667) Drush: Mail
  ...
  civicrm-process-mail-queue` to execute scheduled jobs.
...
- **[CRM-20298](https://issues.civicrm.org/jira/browse/CRM-20298) Drupal Views:
  ...
  This change makes it so that in drupal views, custom fields of type money use
  the filter operators for Integers (instead of the filter operators for text).

- **Add a drush command to rebuild SQL triggers
  ([545](https://github.com/civicrm/civicrm-drupal/pull/545))**

  This change makes it so that one can use the drush command `drush
  civicrm-sql-rebuild-triggers` to rebuild triggers or dump the SQL commands
  into a file.
