...
- **Add pseudoconstant for payment_processor_id to contributionrecur
  ...
  Alters the schema to add support for payment_processor_id as a pseudoconstant
...
- **Allow viewing of cancelled recurring contributions
  ...
  This change makes it possible to view cancelled recurring contributions thru
  the user interface.

- **Payment.sendconfirmation api - add further tpl variables.
  ([13610](https://github.com/civicrm/civicrm-core/pull/13610) and
  [13609](https://github.com/civicrm/civicrm-core/pull/13609))**

  This change adds a Payment.sendconfirmation API.
...
- **CiviCRM Membership Detail report, add column to display if membership is
  ...
  Adds a column "Primary/Inherited?" to the Membership Detail report.
...
- **[dev/core#749](https://lab.civicrm.org/dev/core/issues/749) MySQL utf8mb4
  ...
  This fixes a bug where on some systems MySQL utf8mb4 compatibility check would
  cause a fatal error so that an exception is thrown instead of a fatal error.
...
- **Remove mcrypt system status check
  ...
  PHP 7.2 does not support mcrypt and it is being deprecated. Before this change
  the status check would respond with the error "mcrypt not present & smtp
  encryption in place" after this change no error is thrown.
...
- **Fix api bug whereby 0 & '0' are not accepted as range parameters for BETWEEN
  ...
  This change fixes a bug when using the api and a filter "Between" where one of
  the parameters is 0 or '0' would result in an error so that one can use a
...
- **Fix Custom post outer div class on event registration form
  ...
  in the post section of the form to use the 1 custom_pre-section and 1
