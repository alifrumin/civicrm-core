...
- **Add billingblock region to event registration thankyou to match contribution
  ...
  This allows us to use the same code in payment processor extensions for the
  "confirm" and "thankyou" section of the payment process for both contributions
  and event registrations. Before this change if one wanted to alter/replace the
  billingblock on event registration confirm/thankyou pages one had to target it
  via jquery or using an alterContent hook, but for contribution thankyou one
  could jus replace the region with one that displays billing information relevant
  to the payment processor. This change makes it so one can use the same method to
...
- **[dev/core#846](https://lab.civicrm.org/dev/core/issues/846) CQ: Refactor
  ...
  Rationalises url variables onto shared parent for recurring contribution
  forms.
...
- **Membership form address fields for payment processors
  ...
  This change standardizes processing and validating address parameters on the
  Membership form.
...
- **Menu config screen improvements
  ...
  This change improves the Menu configuration screen by: making it so that
  changes to the menu (adding or editing menu items) on the admin screen are
  reflected immediately, adding links to related settings pages to reduce
  confusion about how to configure various aspects of the menu bar and fixing
  the broken icon picker and missing CiviCRM icon.
...
- **Configurable menubar color
  ...
  scheme (applying a more contemporary palette), but this creates stress for
...
- **[dev/core#824](https://lab.civicrm.org/dev/core/issues/824) What to do with
  ...
  These changes continue work to clean up the Merge screen by moving toward
...
- **Mutliple activity type filters on activity tab on contact records
  ...
  Change include/exclude activity type filters to be multiple select 2 widgets
  rather than only permitting users to select one activity type with a
  traditional select field.
...
- **Added a DB check to prevent deleting exisiting CiviCRM data from database.
  ...
  CiviCRM tables and requires a user to manually remove any CiviCRM tables
  before continuing to prevent users from accidentally deleting the tables.
...
- **Encourage developers to use .then instead of .done
  ...
  (instead of `.done`) and demonstrates proper error handling.
...
- **[dev/core#561](https://lab.civicrm.org/dev/core/issues/561) Replace
  ...
  transaction date field on the update pending status task.

- **Remove reference to non-iso date format from membership form tests
  ([13965](https://github.com/civicrm/civicrm-core/pull/13965))**

  Minor cleanup of tests to reflect our progress getting rid of jcalendar.
...
- **Deploy PEAR Log package and upgrade to latest version in the process
  ...
  This aims to upgrade our version of the PEAR Log package and deploy it via
  composer now.
...
- **Add hook findDuplicates
  ...
  Adds the hook findDuplicates which provides ability for extensions to
...
- **Use TempTable methods.
  ...
  Move towards using the tempTable class to improve standardization including
  but not limited to the following places: when creating the dedupe table, on
  the contribution detail report, on the bookkeeping report and when debugging.

- **Allow extensions to enable validate.tpl
  ([13961](https://github.com/civicrm/civicrm-core/pull/13961))**

  This change makes it possible for extensions to add validate.tpl which
...
- **Promise Polyfill for older browsers
  ...
  Loads a polyfill for IE and other outdated browsers so we can use native js
...
- **Possible paypal fix to avoid sending 500 errors from ipn triggerred by
  ...
  (500 errors from ipn triggerred by non-recurring payment).
...
- **Covert the CRM_Core_Error::fatal calls to exceptions when trying to access
  ...
  Cleaner error when attempting to access contact photos fails.
...
- **CiviCRM API, lookup state_province_id options based on country parameter if
  ...
  Ensures CiviCRM Address API correctly sets state_province_id for states whose
  name/abbreviation exist in multiple countries.
